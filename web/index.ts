...
const ImagePreview = ({ src, onclick, onload, onerror, loading }: ImagePreviewProps) => {
    ...
    const title = 'Clique para copiar para area de transferência';
    return H('a',
    ...
    return H('div',
        ...
        H('div',
            ...
            H('div',
                ...
                H(Field, {
                    label: `Imagem ${images.length + 1}`,
                    input: H(Button, {
                        label: `Add Imagem ${images.length + 1}`,
                        onclick: () => {
                            const nextImage = images.length === 1
                                ? 'https://cdn.jsdelivr.net/gh/brunobach/logo.bradesco@master/bradesco_red.svg'
                                : '';
